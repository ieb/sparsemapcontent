	...
	public void process(Map<String, Object> config,
		...
		try {
			...
			if ( templatePrioritiesConfig != null ) {
				String[] priorityPairs = StringUtils.split(templatePrioritiesConfig, ",");
			...
			RDFToMap rdfToMap = new RDFToMap(namespaceMapConfig, rdfResourceResolver, config);
			...
			if (templateNamePattern != null) {
				...
				// template
				LOGGER.info("Checking template Pattern {} property {}", templateNamePattern, templateSelectorProperty);
				...
				if (base.containsKey(templateSelectorProperty)) {
					...
					if (o instanceof List) {
						ov = ((List) o).toArray(new Object[0]);
					} else {
						ov = new Object[] {String.valueOf(o)};
					}
					...
					for (Object v : ov) {
						...
						if (templateService
								.checkTemplateExists(testTemplateName)) {
							...
							LOGGER.info("Template {} does not exist ",testTemplateName);
						}
					}
				}
			}
			...
			String contentEncoding = (String) config
					.get(CONFIG_CONTENT_ENCODING);

			if (templateName != null) {
				LOGGER.info("Rendering with {} as the base of the result map.",
						key);
				if (contentType != null) {
				...
				if (fullMap.containsKey(RESULT)) {
					response.getWriter()
							.write(gson.toJson(fullMap.get(RESULT)));
				} else {
