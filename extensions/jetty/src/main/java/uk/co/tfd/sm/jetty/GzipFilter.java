  ...
  private static final Logger LOGGER = LoggerFactory.getLogger(GzipFilter.class);
            ...
            (gzip==null || gzip.booleanValue()) && !"HEAD".equalsIgnoreCase(request.getMethod()))
        {
            ...
            finally
            {
                if (exceptional && !response.isCommitted())
                {
                    ...
                    wrappedResponse.noGzip(true, "Exception");
                }
                else
    ...
    public class GZIPResponseWrapper extends HttpServletResponseWrapper
    {
        ...
        public void setContentType(String ct)
        {
            ...
            {
                noGzip(true, "Wrong Mime Type, "+ct);
            }
        }
        ...
        void noGzip(boolean hard, String message)
        {
          LOGGER.debug("no gzip hard :{} {}",hard, message);
            _noGzip=true;
            if ( hard ) {
              _hardNoGzip = true;
            }
    ...
    void doGzip() {
      if (!_hardNoGzip && !isCommitted()) {
        LOGGER.debug("Setting gzip");
        _noGzip = false;
        if (_gzStream != null) {
          try {
            _gzStream.doGzip();
    ...
    public static class GzipStream extends ServletOutputStream
    {
        ...
        {
          LOGGER.debug("Encoding as GZip content");
