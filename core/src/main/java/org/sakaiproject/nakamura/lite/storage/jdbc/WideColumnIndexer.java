...
public class WideColumnIndexer extends AbstractIndexer implements CachingIndexer {
    ...
    private static final String Q_INDEX_UPDATE = "index_update";
    private static final String Q_INDEX_DELETE = "index_delete";
    private static final String Q_INDEX_INSERT = "index_insert";
    private static final String Q_INDEX_REMOVE = "index_remove";
    ...
    /**
     * Set of keys in query properties that cant be cached, normally because
     * invalidation is too hard.
     */
    private static final Set<String> DONT_CACHE_KEYS = ImmutableSet.of(StorageConstants.ITEMS, StorageConstants.PAGE,
            StorageConstants.SORT);
    ...
    public WideColumnIndexer(JDBCStorageClient jdbcStorageClient, Map<String, String> indexColumnsNames,
            Set<String> indexColumnTypes, Map<String, Object> sqlConfig, StatsService statsService) {
    ...
    public void index(Map<String, PreparedStatement> statementCache, String keySpace, String columnFamily, String key, String rid,
            Map<String, Object> values) throws StorageClientException, SQLException {
        ResultSet rs = null;
        ...
        try {
            ...
            if (!StorageClientUtils.isRoot(key) && getColumnName(keySpace, columnFamily, Content.PARENT_HASH_FIELD) != null) {
            ...
            if (nbatch > 0) {
                ...
                client.checkSlow(columnFamily, Q_INDEX_REMOVE, t,
                        client.getSql(keySpace, columnFamily, JDBCStorageClient.SQL_REMOVE_STRING_COLUMN));
                nbatch = 0;
            }
            ...
            if (nbatch > 0) {
                ...
                client.checkSlow(columnFamily, Q_INDEX_INSERT, t,
                        client.getSql(keySpace, columnFamily, JDBCStorageClient.SQL_INSERT_STRING_COLUMN));
                nbatch = 0;
            }
            ...
            if (removeColumns.size() > 0 && updateColumns.size() == 0) {
                ...
                // complete map
                PreparedStatement deleteWideStringColumn = client.getStatement(keySpace, columnFamily, SQL_DELETE_WIDESTRING_ROW,
                        rid, statementCache);
                deleteWideStringColumn.clearParameters();
                deleteWideStringColumn.setString(1, rid);
                long t = System.currentTimeMillis();
                deleteWideStringColumn.execute();
                client.checkSlow(columnFamily, Q_INDEX_DELETE, t, client.getSql(keySpace, columnFamily, SQL_DELETE_WIDESTRING_ROW));
                ...
                // is stuff to add
                String[] sqlParts = StringUtils.split(client.getSql(keySpace, columnFamily, SQL_UPDATE_WIDESTRING_ROW), ";");
                ...
                client.checkSlow(columnFamily, Q_INDEX_UPDATE, t, finalSql);
                if (n == 0) {
                    // part 0 is the final ,part 1 is the template for column
                    // names,
                    ...
                    finalSql = MessageFormat.format(client.getSql(keySpace, columnFamily, SQL_INSERT_WIDESTRING_ROW),
    ...
    public DisposableIterator<Map<String, Object>> find(final String keySpace, final String columnFamily,
        ...
        DisposableIterator<Map<String, Object>> cachedIterator = cachedFind(keySpace, columnFamily, properties, rawResults,
                cachingManager);
        ...
        if (properties != null && properties.containsKey(StorageConstants.CUSTOM_STATEMENT_SET)) {
            ...
            keys = new String[] { "wide-" + customStatement + "." + keySpace + "." + columnFamily,
                    ...
                    "wide-block-find." + keySpace + "." + columnFamily, "wide-block-find." + columnFamily, "wide-block-find" };
        } else {
            keys = new String[] { "wide-block-find." + keySpace + "." + columnFamily, "wide-block-find." + columnFamily,
                    "wide-block-find" };
        }
        ...
        /*
         * Part 0 basic SQL template; {0} is the where clause {1} is the sort
         * clause {2} is the from {3} is the to record eg select rid from css
         * where {0} {1} LIMIT {2} ROWS {3} Part 1 where clause for non array
         * matches; {0} is the columnName eg {0} = ? Part 2 where clause for
         * array matches (not possible to sort on array matches) {0} is the
         * table alias, {1} is the where clause eg rid in ( select {0}.rid from
         * css {0} where {1} ) Part 3 the where clause for array matches {0} is
         * the table alias eg {0}.cid = ? and {0}.v = ? Part 3 sort clause {0}
         * is the list to sort by eg sort by {0} Part 4 sort elements, {0} is
         * the column, {1} is the order eg {0} {1} Dont include , AND or OR, the
         * code will add those as appropriate.
         */
                    ...
                    if (v instanceof Map) {
                        ...
                        for (Iterator<Entry<String, Object>> subtermsIter = subterms.iterator(); subtermsIter.hasNext();) {
                            ...
                            if (shouldFind(keySpace, columnFamily, subk)) {
                                set = processEntry(statementParts, keySpace, columnFamily, subQuery, parameters, subk, subv, sorts,
                                        set, " OR ");
                            }
                        }
                        ...
                        if (v instanceof Iterable<?>) {
                            for (Object vo : (Iterable<?>) v) {
                                set = processEntry(statementParts, keySpace, columnFamily, whereClause, parameters, k, vo, sorts,
                                        set, " AND ");
                            }
                        } else {
                            set = processEntry(statementParts, keySpace, columnFamily, whereClause, parameters, k, v, sorts, set,
                                    " AND ");
                        }
                    }
        ...
        if (whereClause.length() == 0) {
            return cacheResults(keySpace, columnFamily, properties, new PreemptiveCachedMapIterator(client, keySpace, columnFamily,
                    ImmutableMap.of("rows", (Object) ImmutableList.of()), rawResults, cachingManager));
        }
        ...
        final String sqlStatement = MessageFormat.format(statementParts[SQL_QUERY_TEMPLATE_PART], whereClause.toString(),
                sortClause.toString(), items, offset);
        ...
        try {
            ...
            client.checkSlow(columnFamily, "index_find", System.currentTimeMillis() - qtime, sqlStatement);
            ...
            tpst = null;
            trs = null;
            return client.registerDisposable(cacheResults(keySpace, columnFamily, properties, new PreemptiveCachedMapIterator(
    ...
    private int processEntry(String[] statementParts, String keySpace, String columnFamily, StringBuilder subQuery,
        ...
        if (isColumnArray(keySpace, columnFamily, key)) {
            ...
            if (value instanceof Iterable<?>) {
                ...
                // SQL_WHERE_ARRAY_PART is rid in (select rid from css {0} where
                // {1} )
                ...
                join(subQuery, logicalJoin).append(
                        MessageFormat.format(statementParts[SQL_WHERE_ARRAY_PART], tableName, whereClause));
            }
        } else {
    ...
    private DisposableIterator<Map<String, Object>> cachedFind(String keySpace, String columnFamily,
            Map<String, Object> queryProperties, boolean rawResults, DirectCacheAccess cachingManager) {
        Map<String, CacheHolder> queryCache = client.getQueryCache();
        if (queryCache == null) {
            gets++;
            if (gets % 1000 == 0) {
    ...
    private DisposableIterator<Map<String, Object>> cacheResults(String keySpace, String columnFamily,
            Map<String, Object> queryProperties, CachableDisposableIterator<Map<String, Object>> disposableIterator) {
        Map<String, CacheHolder> queryCache = client.getQueryCache();
        if (queryCache == null) {
