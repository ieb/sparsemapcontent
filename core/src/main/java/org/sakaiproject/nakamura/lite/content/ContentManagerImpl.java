...
public class ContentManagerImpl extends CachingManagerImpl implements ContentManager {
    ...
    private static final Set<String> PROTECTED_FIELDS = ImmutableSet.of(LASTMODIFIED_FIELD, LASTMODIFIED_BY_FIELD, UUID_FIELD,
            PATH_FIELD);
    ...
    public ContentManagerImpl(StorageClient client, AccessControlManager accessControlManager, Configuration config,
            Map<String, CacheHolder> sharedCache, StoreListener eventListener, StatsService statsService) {
    ...
    public boolean exists(String path) {
        ...
        try {
            ...
            statsService.apiCall(ContentManagerImpl.class.getName(), "exists", System.currentTimeMillis() - t);
        }
        return false;
    }
    ...
    public Content get(String path) throws StorageClientException, AccessDeniedException {
        ...
        try {
            ...
            statsService.apiCall(ContentManagerImpl.class.getName(), "get", System.currentTimeMillis() - t);
        }
    ...
    public Iterator<Content> listChildren(String path) throws StorageClientException {
        long t = System.currentTimeMillis();
        try {
            final DisposableIterator<Map<String, Object>> childContent = client.listChildren(keySpace, contentColumnFamily, path,
                    this);
            return new PreemptiveIterator<Content>() {
                ...
                protected boolean internalHasNext() {
                    ...
                    if (content == null) {
                        // this is over the top as a disposable iterator should
                        // close auto
                        childContent.close();
                        super.close();
                        return false;
    ...
    public Iterator<String> listChildPaths(final String path) throws StorageClientException {
        long t = System.currentTimeMillis();
        try {
            final Iterator<Map<String, Object>> childContent = client.listChildren(keySpace, contentColumnFamily, path, this);
                    ...
                    while (childContent.hasNext()) {
                        try {
                            ...
                            if (exists(structureMap)) {
                                ...
                                // this is not that efficient since it requires
                                // the map is
                                // loaded, at the moment I don't have a way
                                // round this with the
            ...
            statsService.apiCall(ContentManagerImpl.class.getName(), "listChildPaths", System.currentTimeMillis() - t);
        }
    }
    ...
    public void triggerRefresh(String path) throws StorageClientException, AccessDeniedException {
        Content c = get(path);
        if (c != null) {
            eventListener.onUpdate(Security.ZONE_CONTENT, path, accessControlManager.getCurrentUserId(), getResourceType(c), false,
                    null, "op:update");
        }
    }
            ...
            try {
                while (all.hasNext()) {
                    ...
                    if (c.containsKey(PATH_FIELD) && !c.containsKey(STRUCTURE_UUID_FIELD)) {
                        ...
                        eventListener.onUpdate(Security.ZONE_CONTENT, (String) c.get(PATH_FIELD), User.ADMIN_USER,
                                getResourceType(c), false, null, "op:update");
                    }
                }
            } finally {
                all.close();
    ...
    public void update(Content excontent) throws AccessDeniedException, StorageClientException {
        ...
        try {
            ...
            // deal with content that already exists, but has been marked as new
            // by merging in the new content.
            ...
            if (content.isNew()) {
                ...
                // fields. This should allow content migration.
                ...
                toSave.put(DELETED_FIELD, new RemoveProperty()); // make certain
                                                                 // the
                                                                 // deleted
                                                                 // field is
                                                                 // not set
            ...
            if (content.isNew()) {
                ...
                putCached(keySpace, contentColumnFamily, path,
                        ImmutableMap.of(STRUCTURE_UUID_FIELD, (Object) id, PATH_FIELD, path, DELETED_FIELD, new RemoveProperty()),
                        true);
            } else {
                ...
                if (!exists(structure)) {
                    ...
                    isnew = true;
                    putCached(keySpace, contentColumnFamily, path, ImmutableMap.of(STRUCTURE_UUID_FIELD, (Object) id, PATH_FIELD,
                            path, DELETED_FIELD, new RemoveProperty()), true);
                }
            ...
            eventListener.onUpdate(Security.ZONE_CONTENT, path, accessControlManager.getCurrentUserId(), getResourceType(content),
                    isnew, originalProperties, "op:update");
        } finally {
            statsService.apiCall(ContentManagerImpl.class.getName(), "update", System.currentTimeMillis() - t);
        }
        ...
        try {
            ...
            if (exists(structure)) {
                ...
                if (exists(content)) {
                    ...
                    putCached(keySpace, contentColumnFamily, uuid, ImmutableMap.of(DELETED_FIELD, (Object) TRUE), false);
                    eventListener.onDelete(Security.ZONE_CONTENT, path, accessControlManager.getCurrentUserId(), resourceType,
                            contentBeforeDelete);
                }
                // at 1.4 the deleted field was not on the structure object,
                // this will auto migrate content.
                putCached(keySpace, contentColumnFamily, path, ImmutableMap.of(DELETED_FIELD, (Object) TRUE), false);
            }
        } finally {
            statsService.apiCall(ContentManagerImpl.class.getName(), "delete", System.currentTimeMillis() - t);
        }
    }
    ...
    public long writeBody(String path, InputStream in, String streamId) throws StorageClientException, AccessDeniedException,
            IOException {
        long t = System.currentTimeMillis();
        try {
            checkOpen();
            ...
            Map<String, Object> metadata = client.streamBodyIn(keySpace, contentColumnFamily, contentId, contentBlockId, streamId,
                    content, in);
            metadata.put(StorageClientUtils.getAltField(BODY_LAST_MODIFIED_FIELD, streamId), System.currentTimeMillis());
            metadata.put(StorageClientUtils.getAltField(BODY_LAST_MODIFIED_BY_FIELD, streamId),
                    accessControlManager.getCurrentUserId());
            if (isnew) {
                metadata.put(StorageClientUtils.getAltField(BODY_CREATED_FIELD, streamId), System.currentTimeMillis());
            ...
            eventListener.onUpdate(Security.ZONE_CONTENT, path, accessControlManager.getCurrentUserId(), getResourceType(content),
                    false, null, "stream", streamId);
            return length;
        } finally {
            statsService.apiCall(ContentManagerImpl.class.getName(), "writeBody", System.currentTimeMillis() - t);
        }

    }

    public InputStream getInputStream(String path) throws StorageClientException, AccessDeniedException, IOException {
        return getInputStream(path, null);
    }

    public InputStream getInputStream(String path, String streamId) throws StorageClientException, AccessDeniedException,
            IOException {
        long t = System.currentTimeMillis();
        try {
            checkOpen();
            ...
            statsService.apiCall(ContentManagerImpl.class.getName(), "getInputStream", System.currentTimeMillis() - t);
        }
    }

    private InputStream internalGetInputStream(String contentId, String streamId) throws StorageClientException,
            AccessDeniedException, IOException {
        Map<String, Object> content = getCached(keySpace, contentColumnFamily, contentId);
        if (!exists(content)) {
            return null;
        }
        String contentBlockId = (String) content.get(StorageClientUtils.getAltField(BLOCKID_FIELD, streamId));
        return client.streamBodyOut(keySpace, contentColumnFamily, contentId, contentBlockId, streamId, content);
    }
    ...
    public void move(String from, String to) throws AccessDeniedException, StorageClientException {
        ...
        try {
            ...
            if (exists(toStructure)) {
                ...
                Map<String, Object> content = getCached(keySpace, contentColumnFamily, contentId);
                if (exists(content)) {
                    throw new StorageClientException("The destination content to move to " + to + "  exists, move operation failed");
                }
            }
    ...
    public List<ActionRecord> moveWithChildren(String from, String to) throws AccessDeniedException, StorageClientException {
        ...
        while (iter.hasNext()) {
            ...
            // Since this is a direct child of the previous from, only the last
            // token needs to
            // be appended to "to"
            record.addAll(moveWithChildren(childPath, to.concat(childPath.substring(childPath.lastIndexOf("/")))));
        }
    ...
    public String saveVersion(String path) throws StorageClientException, AccessDeniedException {
        ...
        try {
            ...
            // versionHistoryId is the UUID of the version history for this
            // node.
            ...
            putCached(keySpace, contentColumnFamily, versionHistoryId, ImmutableMap.of(saveVersionId, versionNumber), true);
            putCached(keySpace, contentColumnFamily, path, ImmutableMap.of(STRUCTURE_UUID_FIELD, (Object) newVersionId), true);
            ...
            statsService.apiCall(ContentManagerImpl.class.getName(), "saveVersion", System.currentTimeMillis() - t);
        }
    }

    public List<String> getVersionHistory(String path) throws AccessDeniedException, StorageClientException {
        long t = System.currentTimeMillis();
        try {
            checkOpen();
            accessControlManager.check(Security.ZONE_CONTENT, path, Permissions.CAN_READ);
            ...
            if (exists(structure)) {
                ...
                if (exists(content)) {
                    ...
                    if (versionHistoryId != null) {
                        final Map<String, Object> versionHistory = Maps.newHashMap(getCached(keySpace, contentColumnFamily,
                                versionHistoryId));
            ...
            statsService.apiCall(ContentManagerImpl.class.getName(), "getVersionHistory", System.currentTimeMillis() - t);
        }
    }
    ...
    public Content getVersion(String path, String versionId) throws StorageClientException, AccessDeniedException {
        long t = System.currentTimeMillis();
        try {
            checkOpen();
            accessControlManager.check(Security.ZONE_CONTENT, path, Permissions.CAN_READ);
            ...
            if (exists(structure)) {
                ...
                if (exists(content)) {
                    ...
                    if (versionHistoryId != null) {
                        Map<String, Object> versionHistory = getCached(keySpace, contentColumnFamily, versionHistoryId);
                        if (versionHistory != null && versionHistory.containsKey(versionId)) {
                            Map<String, Object> versionContent = getCached(keySpace, contentColumnFamily, versionId);
                            if (versionContent != null && versionContent.size() > 0) {
                                ...
                                LOGGER.debug("No Content for path {} version History Null{} ", path, versionHistoryId);

                            }
                        } else {
                            LOGGER.debug("History null for path {} version History {} {} ", new Object[] { path, versionHistoryId,
                                    versionHistory });
                        }
                    } else {
            ...
            statsService.apiCall(ContentManagerImpl.class.getName(), "getVersionCollection", System.currentTimeMillis() - t);
        }
    }
    ...
    public Iterable<Content> find(Map<String, Object> searchProperties) throws StorageClientException, AccessDeniedException {
        long t = System.currentTimeMillis();
        try {
            checkOpen();
            ...
            return new Iterable<Content>() {
                ...
                public Iterator<Content> iterator() {
                    ...
                    try {
                        final DisposableIterator<Map<String, Object>> clientSearchKeysIterator = client.find(keySpace,
                                contentColumnFamily, finalSearchProperties, ContentManagerImpl.this);
            ...
            statsService.apiCall(ContentManagerImpl.class.getName(), "find", System.currentTimeMillis() - t);
        }
    }
    ...
    public int count(Map<String, Object> countSearch) throws StorageClientException {
        ...
        try {
            ...
            statsService.apiCall(ContentManagerImpl.class.getName(), "count", System.currentTimeMillis() - t);
        }
    }
