...
public abstract class CachingManagerImpl implements DirectCacheAccess {
     ...
     * 
     * @param client
     *            a client to the underlying storage engine
     * @param sharedCache
     *            the cache where the objects will be stored
     */
    public CachingManagerImpl(StorageClient client, Map<String, CacheHolder> sharedCache, StatsService statsService) {
    ...
    private long getManagerId() {
        // needs to have a low probability of clashing with any other Cache
        // manager in the cluster.
        // no idea what the probability of a clash is here, although I assume
        // its lowish.
        return secureRandom.nextLong();
    }

    /**
     * Try to retrieve an object from the cache. Has the side-effect of loading
     * an uncached object into cache the first time.
     * 
     * @param keySpace
     *            the key space we're operating in.
     * @param columnFamily
     *            the column family for the object
     * @param key
     *            the object key
    ...
    /**
     * Combine the parameters into a key suitable for storage and lookup in the
     * cache.
     * 
     * @param keySpace
     * @param columnFamily
