...
public class NonBatchInsertIndexer extends KeyValueIndexer {
    ...
    public void index(Map<String, PreparedStatement> statementCache, String keySpace, String columnFamily, String key, String rid,
        ...
        for (Entry<String, Object> e : values.entrySet()) {
            ...
            if (shouldIndex(keySpace, columnFamily, k)) {
                if (o instanceof RemoveProperty || o == null) {
                    ...
                    client.checkSlow(columnFamily, Q_INDEX_REMOVE, System.currentTimeMillis() - t,
                    ...
                    if (nrows == 0) {
                        ...
                        LOGGER.debug("Column Not present did not remove {} {} Current Column:{} ", new Object[] { rowId, k, m });
                    } else {
                        LOGGER.debug("Removed Index {} {} {} ", new Object[] { rowId, k, nrows });
                    }
                } else {
                    PreparedStatement removeStringColumn = client.getStatement(keySpace, columnFamily,
                            JDBCStorageClient.SQL_REMOVE_STRING_COLUMN, rid, statementCache);
                    ...
                    for (Object ov : os) {
                        ...
                        nrows = insertStringColumn.executeUpdate();
                        client.checkSlow(columnFamily, Q_INDEX_INSERT, System.currentTimeMillis() - t,
                                client.getSql(keySpace, columnFamily, JDBCStorageClient.SQL_INSERT_STRING_COLUMN));
                        if (nrows == 0) {
                            throw new StorageClientException("Failed to save " + rowId + "  column:[" + k + "] ");
                        } else {
                            LOGGER.debug("Inserted Index {} {} [{}]", new Object[] { rowId, k, v });
                        }
                    }
                }
            }
        ...
        if (!StorageClientUtils.isRoot(key)) {
            ...
            Map<String, Object> autoIndexMap = ImmutableMap.of(Content.PARENT_HASH_FIELD, (Object) hash);
