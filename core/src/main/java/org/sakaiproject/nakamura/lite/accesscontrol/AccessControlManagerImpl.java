...
public class AccessControlManagerImpl extends CachingManagerImpl implements AccessControlManager {
    ...
    public AccessControlManagerImpl(StorageClient client, User currentUser, Configuration config,
            ...
            PrincipalValidatorResolver principalValidatorResolver, StatsService statsService) throws StorageClientException {
    ...
    public Map<String, Object> getAcl(String objectType, String objectPath) throws StorageClientException, AccessDeniedException {
        ...
        try {
            ...
            statsService.apiCall(AccessControlManagerImpl.class.getName(), "getAcl", System.currentTimeMillis() - t);
        }
    }
    ...
    public PropertyAcl getPropertyAcl(String objectType, String objectPath) throws AccessDeniedException, StorageClientException {
        ...
        try {
            ...
            // if the property has been granted, then that should remove the
            // deny
    ...
    public Map<String, Object> getEffectiveAcl(String objectType, String objectPath) throws StorageClientException,
    ...
    public void setAcl(String objectType, String objectPath, AclModification[] aclModifications) throws StorageClientException,
            AccessDeniedException {
        long t = System.currentTimeMillis();
        try {
            checkOpen();
            ...
            statsService.apiCall(AccessControlManagerImpl.class.getName(), "setAcl", System.currentTimeMillis() - t);
        }
    }
    ...
    public void check(String objectType, String objectPath, Permission permission) throws AccessDeniedException,
    ...
    private int[] compilePermission(Authorizable authorizable, String objectType, String objectPath, int recursion)
        ...
        try {
            // we need to allow the permissions compile to bypass access control
            // as it needs to see everything.
            compilingPermissions.inc();
            ...
            if (acl != null) {

                {
                    ...
                    grants = grants | tg;
                    denies = denies | td;
                    LOGGER.debug("Added Permissions for {} g{} d{} => g{} d{}", new Object[] { principal, tg, td, grants, denies });
                        ...
                        if (secretKey != null) {
                            for (Entry<String, Object> ace : acl.entrySet()) {
                                ...
                                if (k.startsWith(DYNAMIC_PRINCIPAL_STEM)) {
                                    String proxyPrincipal = AclModification.getPrincipal(k).substring(
                                            DYNAMIC_PRINCIPAL_STEM.length());
                                    if (!inspected.contains(proxyPrincipal)) {
                                        inspected.add(proxyPrincipal);
                                        LOGGER.debug("Is Dynamic {}, checking ", k);
                                        try {
                                            // principalTokenValidators are not
                                            // safe code, hence we must
                                            // re-enable full access control.
                                            compilingPermissions.suspend();
                                            List<Content> proxyPrincipalTokens = principalTokenResolver
                                                    .resolveTokens(proxyPrincipal);
                                            for (Content proxyPrincipalToken : proxyPrincipalTokens) {
                                                if (principalTokenValidator.validatePrincipal(proxyPrincipalToken, secretKey)) {
                                                    ...
                                                    grants = grants | tg;
                                                    denies = denies | td;
                                                    LOGGER.debug("Added Permissions for {} g{} d{} => g{} d{}", new Object[] {
                                                            pname, tg, td, grants, denies });
                                                    break;
                                                }
                                            }
                                        } finally {
                                            // when done, we must resume
                                            // compiling permissions where we
                                            // were.
                ...
                if (!User.ANON_USER.equals(authorizable.getId())) {
                    // all users except anon are in the group everyone, by
                    // default
                    // but only if not already denied or granted by a more
                    // specific
                    // permission.
                    ...
                    grants = grants | tg;
                    denies = denies | td;
                    LOGGER.debug("Added Permissions for {} g{} d{} => g{} d{}", new Object[] { Group.EVERYONE, tg, td, grants,
                            denies });
                ...
                /*
                 * Only look to parent objects if this is not the root object
                 * and everything is not granted and denied
                 */
                if (recursion < 20 && !StorageClientUtils.isRoot(objectPath) && (granted != 0xffff || denied != 0xffff)) {
                    recursion++;
                    ...
                    if (parentPriv != null) {
                         ...
                         * ~denies is permissions not denied here parentPriv[0]
                         * & ~denies is permissions granted by the parent that
                         * have not been denied here. we need to add those to
                         * things granted here. ie |
                         */
                ...
                if (((denied & Permissions.CAN_READ.getPermission()) == 0)
                        && (Security.ZONE_AUTHORIZABLES.equals(objectType) || Security.ZONE_CONTENT.equals(objectType))) {
    ...
    private String[] getPrincipals(final Authorizable authorizable) {
        ...
        if (authorizableManager != null) {
            // membership resolution is possible, but we had better turn off
            // recursion
