...
public class BatchInsertIndexer extends KeyValueIndexer {
    ...
    private static final String Q_BATCH_REMOVE = "batch_remove";
    ...
    public void index(Map<String, PreparedStatement> statementCache, String keySpace, String columnFamily, String key, String rid,
        ...
        for (Entry<String, Object> e : values.entrySet()) {
            ...
            if (shouldIndex(keySpace, columnFamily, k)) {
                if (o instanceof RemoveProperty || o == null) {
                    ...
                    removeStringColumn.addBatch();
                    removeSet.add(removeStringColumn);
                    // insert new values, as we just removed them we know we can
                    // insert, no need to attempt update
                    // the only thing that we know is the colum value changes so
                    // we have to re-index the whole
                    // property
        ...
        if (!StorageClientUtils.isRoot(key)) {
            // create a holding map containing a rowhash of the parent and then
            // process the entry to generate a update operation.
            Map<String, Object> autoIndexMap = ImmutableMap.of(Content.PARENT_HASH_FIELD,
                    (Object) client.rowHash(keySpace, columnFamily, StorageClientUtils.getParentObjectPath(key)));
        ...
        for (PreparedStatement pst : removeSet) {
            ...
            pst.executeBatch();
            client.checkSlow(columnFamily, Q_BATCH_REMOVE, System.currentTimeMillis() - t, Q_BATCH_REMOVE);
        }
        ...
        for (PreparedStatement pst : insertSet) {
            ...
            for (int i = 0; i < res.length; i++) {
                ...
                if (res[i] <= 0 && res[i] != -2) { // Oracle drivers respond
                    // with -2 on a successful
                    // insert when the number is
                    // not known
                    ...
                    LOGGER.debug("Index inserted for {} {} ", new Object[] { rid, e.getKey(), e.getValue() });
