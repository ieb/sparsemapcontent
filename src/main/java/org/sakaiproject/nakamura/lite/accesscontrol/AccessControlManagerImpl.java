...
public class AccessControlManagerImpl implements AccessControlManager  {
	...
	public AccessControlManagerImpl(StorageClient client, User currentUser,
	...
	public byte[] toByteMap(int bitmap) {
		return StorageClientUtils.toBytes(Integer.toHexString(bitmap));
	}
	...
	public void setAcl(String objectType, String objectPath,
			AclModification[] aclModifications) throws StorageClientException,
		...
		for (AclModification m : aclModifications) {
			...
			if (m.isRemove()) {
				...
				modifications.put(name, toByteMap(bitmap));
			}
		}
	...
	public void check(String objectType, String objectPath, Permission permission)
		...
		if (!((permission.getPermission() & privileges[0]) == permission
				.getPermission())) {
			throw new AccessDeniedException(objectType, objectPath,
	...
	private int[] compilePermission(String objectType, String objectPath,
			int recursion) {
		...
		if (acl != null) {
			for (String principal : user.getPrincipals()) {
				grants = grants
						| toBitmap((byte[]) acl.get(principal + GRANTED_MARKER));
				denies = denies
						| toBitmap((byte[]) acl.get(principal + DENIED_MARKER));
			}
			/*
