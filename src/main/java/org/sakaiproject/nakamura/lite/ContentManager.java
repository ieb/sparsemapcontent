...
public class ContentManager {
	...
	/**
	 * List of Child names presented to the client
	 */
	...
	public Map<String, Object> get( String path) throws StorageClientException, AccessDeniedException {
		...
		String contentId = StorageClientUtils.toString((byte[]) structure.get(UUID_FIELD));
		Map<String, Object> content = client.get(keySpace, contentColumnFamily, contentId);
		
		Map<String, Object> children = Maps.newHashMap();
		for ( Entry<String,Object> child : structure.entrySet()) {
			String k = child.getKey();
			if ( !k.startsWith(":") ) {
				children.put(k, child.getValue());
			}
		}
		content.put(CHILDREN_FIELD, children);
		return content;
	}
	...
	public void saveVersion(String path) throws StorageClientException, AccessDeniedException {
		...
		Map<String, Object> structure = client.get(keySpace, contentColumnFamily, path);
		String contentId = StorageClientUtils.toString((byte[]) structure.get(STRUCTURE_UUID_FIELD));
		...
		String saveVersionId = StorageClientUtils.toString((byte[]) saveVersion.get(UUID_FIELD));
		String saveBlockId = StorageClientUtils.toString((byte[]) saveVersion.get(BLOCKID_FIELD));

		byte[] newVersionIdB = StorageClientUtils.toBytes(newVersionId);
		byte[] saveVersionIdB = StorageClientUtils.toBytes(saveVersionId);
		byte[] saveBlockIdB = StorageClientUtils.toBytes(saveBlockId);

		newVersion.put(UUID_FIELD, newVersionIdB);
		newVersion.put(PREVIOUS_VERSION_UUID_FIELD, saveVersionIdB);
		newVersion.put(PREVIOUS_BLOCKID_FIELD, saveBlockIdB);
		
		saveVersion.put(NEXT_VERSION_FIELD, newVersionIdB);
		...
		client.insert(keySpace, contentColumnFamily, path, ImmutableMap.of( STRUCTURE_UUID_FIELD, (Object)newVersionIdB));
		client.insert(keySpace, contentColumnFamily, StorageClientUtils.getParentObjectPath(path), ImmutableMap.of( StorageClientUtils.getObjectName(path), (Object)newVersionIdB));
		
	}
	
	
	public void update(String path, Map<String, Object> content) throws AccessDeniedException, StorageClientException {
		accessControlManager.check(Security.ZONE_CONTENT, path, Permissions.CAN_WRITE);
		String id = null;
		byte[] idb = null;
		if ( content.containsKey(UUID_FIELD) ) {
			id = StorageClientUtils.toString(content.get(UUID_FIELD));
			idb = StorageClientUtils.toBytes(content.get(UUID_FIELD));
		} else {
			id = StorageClientUtils.getUuid();
			idb = StorageClientUtils.toBytes(id);
			content.put(UUID_FIELD, idb);
		}
		...
		content.put(PATH_FIELD, StorageClientUtils.toBytes(path));
		client.insert(keySpace, contentColumnFamily, StorageClientUtils.getParentObjectPath(path), ImmutableMap.of( StorageClientUtils.getObjectName(path), (Object)idb));
		client.insert(keySpace, contentColumnFamily, path, ImmutableMap.of( STRUCTURE_UUID_FIELD, (Object)idb));
		client.insert(keySpace, contentColumnFamily, id, content);
	}
	...
	public void delete(String path) throws AccessDeniedException, StorageClientException {
		...
		client.insert(keySpace, contentColumnFamily, StorageClientUtils.getParentObjectPath(path), ImmutableMap.of(StorageClientUtils.getObjectName(path), null));
		client.insert(keySpace, contentColumnFamily, uuid, ImmutableMap.of( DELETED_FIELD, (Object)TRUE_B));
		client.insert(keySpace, contentColumnFamily, DELETEDITEMS_KEY, ImmutableMap.of(uuid, (Object)StorageClientUtils.toBytes(path)));
	}
	...
	public long writeBody(String path, InputStream in) throws StorageClientException, AccessDeniedException, IOException {
		...
		if ( content.containsKey(BLOCKID_FIELD) ) {
			contentBlockId = StorageClientUtils.toString((byte[]) content.get(BLOCKID_FIELD));
		} else {
		...
		for(;;) {
			...
			lastBlockWrite = i;
			client.insert(keySpace, contentColumnFamily, key, ImmutableMap.of( UUID_FIELD, (Object)StorageClientUtils.toBytes(contentId), 
					NUMBLOCKS_FIELD, StorageClientUtils.toBytes(bodyNum), 
					blockLengthKey, StorageClientUtils.toBytes(bufferLength), 
					bodyKey, saveBuffer ));
			bodyNum++;
		...
		client.insert(keySpace, contentColumnFamily, contentId, ImmutableMap.of(
				BLOCKID_FIELD, (Object)StorageClientUtils.toBytes(contentBlockId), 
				NBLOCKS_FIELD, StorageClientUtils.toBytes(lastBlockWrite),
				LENGTH_FIELD, StorageClientUtils.toBytes(length),
				BLOCKSIZE_FIELD, StorageClientUtils.toBytes(BLOCK_SIZE)));
		return length;
		
	}
        
	public InputStream getInputStream(String path) throws StorageClientException, AccessDeniedException {
	...
	public class ContentInputStream extends InputStream {
		...
		private boolean readBuffer() throws IOException {
			...
			blockLength = StorageClientUtils.toInt(block.get(BLOCK_LENGTH_FIELD_STUB+currentBlockNumber));
