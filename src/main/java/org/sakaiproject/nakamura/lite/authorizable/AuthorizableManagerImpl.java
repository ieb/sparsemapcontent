...
public class AuthorizableManagerImpl implements AuthorizableManager {
	...
	private static final Set<String> FILTER_ON_UPDATE = ImmutableSet.of(
			Authorizable.ID_FIELD, Authorizable.PASSWORD_FIELD);
	private static final Set<String> FILTER_ON_CREATE = ImmutableSet.of(
			Authorizable.ID_FIELD, Authorizable.PASSWORD_FIELD);
	private static final Logger LOGGER = LoggerFactory
			.getLogger(AuthorizableManagerImpl.class);
	...
	public AuthorizableManagerImpl(User currentUser, StorageClient client,
		...
		this.authorizableColumnFamily = configuration
	...
	public Authorizable findAuthorizable(String authorizableId)
		...
		if (!this.currentUserId.equals(authorizableId)) {
			accessControlManager.check(Security.ZONE_AUTHORIZABLES,
					authorizableId, Permissions.CAN_READ);
		}
		Map<String, Object> authorizableMap = client.get(keySpace,
				authorizableColumnFamily, authorizableId);
	...
	public void updateAuthorizable(Authorizable authorizable)
		...
		accessControlManager.check(Security.ZONE_AUTHORIZABLES, id,
		 ...
		 * added to the list of principals.
		 * All Authorizables that require modification are then modified in the store.
		 * Write permissions to modify principals is granted as a result of being able to read the authorizable and being able to add the member.
		 * FIXME:  possibly we might want to consider using a "can add this user to a group" permission at some point in the future.
		 */
				...
				if (newMember != null) {
					...
					if (newMember.isModified()) {
						Map<String, Object> encodedProperties = StorageClientUtils
								.getFilteredAndEcodedMap(
										newMember.getPropertiesForUpdate(),
										FILTER_ON_UPDATE);
						client.insert(keySpace, authorizableColumnFamily,
								authorizable.getId(), encodedProperties);
					}
				}
			}
			for (Authorizable retiredMember : retiredMembers) {
				if (retiredMember != null) {
					...
					if (retiredMember.isModified()) {
						Map<String, Object> encodedProperties = StorageClientUtils
								.getFilteredAndEcodedMap(
										retiredMember.getPropertiesForUpdate(),
										FILTER_ON_UPDATE);
						client.insert(keySpace, authorizableColumnFamily,
								authorizable.getId(), encodedProperties);
					}
		...
		Map<String, Object> encodedProperties = StorageClientUtils
				.getFilteredAndEcodedMap(authorizable.getPropertiesForUpdate(),
						FILTER_ON_UPDATE);
	...
	public boolean createAuthorizable(String authorizableId,
			String authorizableName, String password,
			Map<String, Object> properties) throws AccessDeniedException,
			StorageClientException {
		if (Authorizable.isAGroup(properties)) {
			accessControlManager.check(Security.ZONE_ADMIN,
					Security.ADMIN_GROUPS, Permissions.CAN_WRITE);
		} else {
			accessControlManager.check(Security.ZONE_ADMIN,
					Security.ADMIN_USERS, Permissions.CAN_WRITE);
		}
		...
		Map<String, Object> encodedProperties = StorageClientUtils
				.getFilteredAndEcodedMap(properties, FILTER_ON_CREATE);
		encodedProperties.put(Authorizable.ID_FIELD,
				StorageClientUtils.toBytes(authorizableId));
		encodedProperties.put(Authorizable.NAME_FIELD,
				StorageClientUtils.toBytes(authorizableName));
		if (password != null) {
			encodedProperties.put(Authorizable.PASSWORD_FIELD,
					StorageClientUtils.toBytes(StorageClientUtils
							.secureHash(password)));
		} else {
			encodedProperties.put(Authorizable.PASSWORD_FIELD,
					StorageClientUtils.toBytes(Authorizable.NO_PASSWORD));
		}
		client.insert(keySpace, authorizableColumnFamily, authorizableId,
				encodedProperties);
		return true;
	}

	public boolean createUser(String authorizableId, String authorizableName,
		...
		return createAuthorizable(authorizableId, authorizableName, password,
				properties);
	}
	...
	public boolean createGroup(String authorizableId, String authorizableName,
		...
		return createAuthorizable(authorizableId, authorizableName, null,
				properties);
	}
