...
public class ClientConnection extends Client implements StorageClient {
	...
	public void destroy() {
		try {
			...
			LOGGER.error(
					"Failed to close the connection to the cassandra store.", e);
		}
	}
	...
	public Map<String, Object> get(String keySpace, String columnFamily,
		...
		try {
			...
			List<ColumnOrSuperColumn> results = get_slice(keySpace, key,
	...
	public void insert(String keySpace, String columnFamily, String key,
		...
		try {
			...
			for (Entry<String, Object> value : values.entrySet()) {
				...
				List<Mutation> keyMutations = getMutationList(columnMutations,
						name);
				...
				if (v == null) {
					...
					byte[] bv = (byte[]) v;
					Column column = new Column(bname, bv,
					...
					for (Entry<String, byte[]> sce : sc.entrySet()) {
						...
						Column column = new Column(bcname, sce.getValue(),
								System.currentTimeMillis());
						columns.add(column);
					}
	...
	private List<Mutation> getMutationList(
			Map<String, List<Mutation>> columnMutations, String key) {
